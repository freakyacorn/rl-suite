...
class ChargeSequence(Environment, ABC):
    ...
    def get_possible_actions(self, states=None):
        ...
        for i, s in enumerate(states):
            valid_actions = (
                self.action_list
            )  # All actions are valid for this environment
    ...
    def load_state_dict(self, state_dict):
        ...
        self.data_files = [
            ChargeSequenceData(filename) for filename in state_dict["data_files"]
        ]
...
class SCDChargeSequence(ChargeSequence):
    ...
    def modify_simulations(self, actions):
        ...
        self.states = [
            [state[0] + action[0]] for state, action in zip(self.states, actions)
        ]

        # Generate new charge sequence based on the SCD
        charges = Parallel(n_jobs=-1)(
            delayed(_generate_SCD)(scd[0]) for scd in self.states
        )
...
def _generate_SCD(target_scd):
    ...
    final_sequence, final_scd = _simulated_annealing(
        sequence, timesteps=timesteps, T=T, target=target_scd
    )
    charge_dict = {"R": 1, "D": -1}
