...
class LjTemperature(Environment):
    ...
    def __init__(self, target, action_list, n_simulations=100, sim_steps_per_round=1000000,
    ...
    def initialize_simulations(self):
        ...
        for i in range(self.n_simulations):
            ...
            subprocess.run(['cp', '-f', os.path.join(self.data_dir, 'sys.data'), poly_dir])
        ...
        for i, (T, seed) in enumerate(zip(self.temperatures, self.seeds)):
            ...
            _write_lmp(path, self.sim_steps_per_round, T, seed, self.dump_style, rewrite=False)
    ...
    def modify_simulations(self, actions):
        ...
        for i, (T, seed) in enumerate(zip(self.temperatures, self.seeds)):
            path = os.path.join(self.simulation_dir, f'poly{i}/start.lmp')
            _write_lmp(path, self.sim_steps_per_round, T, seed, self.dump_style, rewrite=True)
    ...
    def get_possible_actions(self, states=None):
        ...
        for i, T in enumerate(states):
            valid_actions = [action for action in self.action_list if 100 <= T + action <= 600]
