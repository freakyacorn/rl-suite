...
class PNAFolding(Environment, ABC):
    ...
    def __init__(self, sequence_file, target, action_list, n_simulations=112, sim_steps_per_round=1000000,
    ...
    def modify_simulations(self, actions):
        ...
        self.temperatures = [action[0] for action in actions]  # Assuming actions is a list of lists with one action each
            ...
            with open(f'{self.simulation_dir}/poly{i}/in.pna', 'w') as file:
                for line in lines:
                    if line.startswith('variable            T0'):
                        file.write(f'variable            T0             index    {self.temperatures[i]}\n')
                    else:
                        file.write(line)
...
class AllOrNothingPNAFolding(PNAFolding):
    ...
    def calculate_rewards(self):
        ...
        temperature_penalty = [-1 * (max(0, t - 10) + max(0, 2.5 - t)) for t in self.temperatures]
        return [s[1] + temperature_penalty[i] for i, s in enumerate(self.states)]  # Assuming states are structured as [[state_value], ...]
    ...
    def calculate_additional_metrics(self):
        ...
        additional_metrics = {}
        additional_metrics['temperatures'] = [[t] for t in self.temperatures]  # Wrap in list to match state structure
        additional_metrics['percent_on_target'] = [[sum(self.calculate_rewards()) / self.n_simulations * 100]]
        additional_metrics['pna_cv'] = [s[1:] for s in self.states]  # Extract PNA CVs from states
...
class IncrementalPNAFolding(PNAFolding):
    ...
    def calculate_rewards(self):
        ...
        for i, (state, classification) in enumerate(zip(pna_cv, classified_states)):
            ...
            rewards[i] += -100 * (max(0, self.temperatures[i] - 10) + max(0, 2.5 - self.temperatures[i]))
    ...
    def calculate_additional_metrics(self):
        ...
        additional_metrics = {}
        additional_metrics['temperatures'] = [[t] for t in self.temperatures]  # Wrap in list to match state structure
        classified_states = [classify_pna(state[1:], self.target, cutoff=5.0) for state in self.states]
...
class GroupedPNAFolding(PNAFolding):
    ...
    def __init__(self, sequence_file, target, action_list, n_simulations=112, 
                 sim_steps_per_round=1000000, dump_style='dcd', 
        ...
        '''
        super().__init__(sequence_file, target, action_list, 
                         n_simulations, sim_steps_per_round, 
                         dump_style, simulation_dir)
    ...
    def initialize_simulations(self):
        ...
        for i in range(self.n_simulations):
            ...
            commands.append(['python', '/home/wo6860/projects/pna/sum2024_project/generate.py', 
                             self.sequence_file,
                            '--output', poly_dir,
                            '--temperature', str(self.temperatures[i // self.group_size]),
                            '--nsteps', str(self.sim_steps_per_round),
                            '--seed', str(self.seeds[i]),
                            '--coords_freq', '10000',
                            '--dump_style', self.dump_style,
    ...
    def modify_simulations(self, actions):
        ...
        for i in range(self.n_simulations):
            ...
            with open(f'{self.simulation_dir}/poly{i}/in.pna', 'w') as file:
                for line in lines:
                    if line.startswith('variable            T0'):
                        file.write(f'variable            T0             index    {self.temperatures[i // self.group_size]}\n')
                    else:
                        file.write(line)
    ...
    def calculate_states(self):
        ...
        self.states = [[float(t)] + [float(s)] for t, s in zip(self.temperatures, group_states)]
        return self.states
    ...
    def calculate_rewards(self):
        ...
        temperature_penalty = [-100 * (max(0, t - 10) + max(0, 2.5 - t)) for t in self.temperatures]
    ...
    def calculate_additional_metrics(self):
        ...
        additional_metrics = {}
        additional_metrics['temperatures'] = [[t] for t in self.temperatures]  # Wrap in list to match state structure
        additional_metrics['percent_on_target'] = [[self.states[i][1] for i in range(self.n_groups)]]
        return additional_metrics
