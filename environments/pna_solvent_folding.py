...
class PNAFolding(Environment, ABC):
    ...
    def initialize_simulations(self):
        ...
        for i, interaction_strength in enumerate(self.interaction_strengths):
            ...
            settings_file_path = os.path.join(
                self.simulation_dir, f"poly{i}", "sys.settings"
            )
            ...
            settings.modify_non_bonded(
                2, 3, "lj/gromacs", [at_strength, 0.3, 0.57, 0.6]
            )
            settings.modify_non_bonded(
                4, 5, "lj/gromacs", [gc_strength, 0.3, 0.57, 0.6]
            )
            settings.write()
    ...
    def modify_simulations(self, actions):
        ...
        self.interaction_strengths = [
            [action[0], 100 - action[0]] for action in actions
        ]

        # Modify lammps settings files to reflect new interactions
        for settings, interaction_strength in zip(
            self.settings_files, self.interaction_strengths
        ):
            at_strength, gc_strength = interaction_strength[0], interaction_strength[1]
            settings.modify_non_bonded(
                2, 3, "lj/gromacs", [at_strength, 0.3, 0.57, 0.6]
...
class IncrementalPNAFolding(PNAFolding):
    ...
    def calculate_additional_metrics(self):
        ...
        additional_metrics["interaction_strengths"] = self.interaction_strengths
        classified_states = [
            classify_pna(state[2:], self.target, cutoff=5.0) for state in self.states
        ]
        additional_metrics["percent_on_target"] = [
...
class GroupedPNAFolding(PNAFolding):
    ...
    def calculate_states(self):
        ...
        self.states = [
            i + [s] for i, s in zip(self.interaction_strengths, group_states)
        ]
        return self.states
    ...
    def calculate_additional_metrics(self):
        ...
        additional_metrics["interaction_strengths"] = self.interaction_strengths
        additional_metrics["percent_on_target"] = [
            [self.states[i][2] for i in range(self.n_groups)]
        ]
        return additional_metrics
