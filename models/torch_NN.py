...
class NeuralNetwork(nn.Module, Model):
    ...
    def __init__(
        ...
        seed=None,
        weight_initialization="xavier_uniform_",
        bias_initialization="zeros_",
        use_batch_norm=True,
        scale_data=False,
    ):
        """
        ...
        if self.use_batch_norm and self.scale_data:
            print(
                "Warning: Are you sure you want to scale the data and use batch normalization?"
            )
    ...
    def get_weights(self):
        ...
        weights = [
            module.weight for module in self.modules() if isinstance(module, nn.Linear)
        ]
        return weights
    ...
    def get_biases(self):
        ...
        biases = [
            module.bias for module in self.modules() if isinstance(module, nn.Linear)
        ]
        return biases
    ...
    def fit(
        ...
        if self.scale_data:
            ...
            train_data = [
                (
                    torch.tensor(
                        self.scaler_X.transform(x.unsqueeze(0)), dtype=torch.float32
                    ).squeeze(0),
                    torch.tensor(
                        self.scaler_y.transform(y.reshape(-1, 1)), dtype=torch.float32
                    ).squeeze(0),
                )
        ...
        for epoch in range(epochs):
            ...
            if verbose:
                print(
                    f"Epoch: {epoch + 1:4d}/{epochs}, Loss: {total_loss / train_size:.5f}, Val Loss: {val_loss / val_size:.5f}"
                )
    ...
    def __repr__(self):
        model_architecture = (
            f"Input Layer: {self.input.in_features} -> {self.input.out_features}\n"
        )
        for i, layer in enumerate(self.hidden_layers):
            model_architecture += (
                f"Hidden Layer {i + 1}: {layer.in_features} -> {layer.out_features}\n"
            )
        model_architecture += (
            f"Output Layer: {self.output.in_features} -> {self.output.out_features}\n"
        )
