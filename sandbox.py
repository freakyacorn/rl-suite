...
class Sandbox:
    def __init__(self, environment, agent, rounds=199, save_dir='checkpoints',
                 visualize=False, vis_dir='visualizations'):
    ...
    def execute_round(self):
        ...
        actions = self.agent.choose_actions(
            self.states,
            self.environment.get_possible_actions()
        )
        ...
        if self.visualize and self.current_round:
            ...
            for key, values in self.results_history.items():
                ...
                if values_array.ndim == 2:
                    ...
                    plt.fill_between(rounds, lower_quartile, upper_quartile, alpha=0.3, label=f"25-75% {key}")
                    plt.xlabel("Round")
                    plt.ylabel(f"{key}")
